...
odoo.define("web_drop_target", function(require) {
    ...
    var DropTargetMixin = {
        ...
        _get_drop_items: function(e) {
            if (this._get_form_state().res_id) {
                var self = this,
                    dataTransfer = e.originalEvent.dataTransfer,
                    drop_items = [];
                _.each(dataTransfer.files, function(item) {
                    var type = item.type || item.name.split('.')[1];
                    if (
                        _.contains(self._drop_allowed_types, type) ||
        ...
        _add_overlay: function() {
            ...
            if (!this._drop_overlay) {
                ...
                var state = this._get_form_state();
        ...
        _get_form_state: function () {
            return this.renderer ? this.renderer.state : this.getParent().state;
        }
    };
        ...
        _.extend(DropTargetMixin, {
            ...
            _handle_drop_items: function(drop_items, e) {
                ...
                _.each(drop_items, function(item, e) {
                    var state = self._get_form_state();
